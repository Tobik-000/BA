...
class MyRISController:
    ...
    def configure(self, voltage_range_identifier: str, dac_idx: int = 0):
        ...
        # --- SAFETY MECHANISM: Set DAC15 to 3V ---
        safety_voltage = 3.0
        full_scale_voltage = self.MAX_VOLTAGES[voltage_range_identifier]
        dac_value = int(
            round(
                (
                    (safety_voltage - self.MIN_VOLTAGES[voltage_range_identifier])
                    / (full_scale_voltage - self.MIN_VOLTAGES[voltage_range_identifier])
                )
                * (2**16 - 1)
            )
        )
        ...
        if self.daisy_chain_device_num == 0:
            self.dac[dac_idx].set_dacn(15, dac_value)
        else:
            # ERROR: Setting DAC15 in daisychain mode is not supported
            raise NotImplementedError(
                "Setting DAC15 in daisychain mode is not supported."
            )
    ...
    def enable_dac_channels(self, dac_idx: int = 0):
        ...
        config = 0xFFFF ^ (((1 << self._unit_cell_num) - 1) | (1 << 15))
        ...
        if self.daisy_chain_device_num == 0:
            ...
            # the config is intentionally missing the repeat as it should already have the correct shape
            mdac.set_register(
                np.repeat(
