...
class MySPI_FTDI4232H():
    ...
    def _rotate_sequence(self, sequence):
        return np.packbits(np.unpackbits(sequence, axis=1, bitorder='big'), axis=0, bitorder='little')
    ...
    """
    def create_data_sequence(self, register_address: int | List[int], byte_high: int | List[int], byte_low: int | List[int]):
        if self._daisy_chain_device_num > 0:
            if not isinstance(register_address, List) or not isinstance(byte_high, List) or not isinstance(byte_low, List):
                raise ValueError(f"Register address, high byte and low byte must be lists with length {self._daisy_chain_device_num + 1}")
            ...
            if reg_add_np.size != (self._daisy_chain_device_num + 1) or byt_hig_np.size != (self._daisy_chain_device_num + 1) or byt_low_np.size != (self._daisy_chain_device_num + 1):
                raise ValueError(f"Register address, high byte and low byte must be lists with length {self._daisy_chain_device_num + 1}")
        else:
            if len(np.shape(np.asarray(register_address))) != 0 or len(np.shape(np.asarray(byte_high))) != 0 or len(np.shape(np.asarray(byte_low))) != 0:
                raise ValueError(f"Register address, high byte and low byte must be a scalar")
            
        clk_seq = np.array(np.repeat(0xAA, 6*(self._daisy_chain_device_num + 1)), dtype=np.uint8)
        ...
        data_seq = np.asarray([register_address, byte_high, byte_low], dtype=np.uint8).reshape(-1, order="F")
    ...
    def _process_rx_data(self, sequence):
        rx_data_unpacked = np.unpackbits(np.asarray(np.frombuffer(bytes(sequence), dtype=np.uint8))[:,None], axis=1, bitorder='little')
        #rx_data_unpacked_clean = rx_data_unpacked[:,:4]
        #print(np.argwhere(rx_data_unpacked_clean[:,1] == 1).squeeze())
        data_segment1 = rx_data_unpacked[2:50:2,:4] # throw out guard and unused pins, downsample received data
        ...
        return (address_segment1_tx, data_segment1_tx, address_segment2_tx, data_segment2_tx), (address_segment1_rx, data_segment1_rx, address_segment2_rx, data_segment2_rx)
        #print(hex((1 << 7) | 0x01))
    ...
    def read(self, address, read_delay: int = 1):
        if self._daisy_chain_device_num > 0:
            raise NotImplementedError("Reading in daisychain mode (for DAC81416-08EVM) is not supported")
        if read_delay < 1:
        ...
        tx_data = np.append(np.append(self._prolog, np.append(sequence, np.append(delay_seq, sequence))), self._epilog)
...
class MyDAC81416:
    ...
    REGISTER_MAP = { # see section 8.6 Register Maps, Table 8-7. in the DACx1416 datasheet
        "NOP":          0x00,
        "DEVICEID":     0x01,
        "STATUS":       0x02,
        "SPICONFIG":    0x03,
    ...
    def get_device_version_id(self):
        (_, _, _, _), (_, _, _, data_segment2_rx) = self.spi.read(self.REGISTER_MAP["DEVICEID"])
        device_id_bin = data_segment2_rx[:14]
        version_id_bin = data_segment2_rx[14:]
        return hex(int("".join(device_id_bin.astype(str)),2)), hex(int("".join(version_id_bin.astype(str)),2))
    
    def get_status(self):
        (_, _, _, _), (_, _, _, data_segment2_rx) = self.spi.read(self.REGISTER_MAP["STATUS"])
    ...
    """
    def set_register(self, address: int | List[int], config: int | List[int], config_mask: int | List[int], reserved_state: int | List[int] = 0):
        ...
        if isinstance(address, int) and isinstance(config, int) and isinstance(config_mask, int):
            if (config ^ reserved_state) & (~config_mask):
                raise ValueError("You are writing to read only bits or did not set the reserved bits correctly!")
        else:
            ...
            if len(addr_np) == len(conf_np) == len(conf_mk_np) == len(rsrv_st_np):
                ...
                raise ValueError("address, config, config_mask, reserved_state must have the same length")
    ...
    def get_spiconfig(self):
        (_, _, _, _), (_, _, _, data_segment2_rx) = self.spi.read(self.REGISTER_MAP["SPICONFIG"])
        ...
        return tempalm_en, dacbusy_en, crcalm_en, softtoggle_en, dev_pwdwn, crc_en, str_en, sdo_en, fsdo
    ...
    def get_genconfig(self):
        (_, _, _, _), (_, _, _, data_segment2_rx) = self.spi.read(self.REGISTER_MAP["GENCONFIG"])
        ...
        return ref_pwdwn, dac_14_15_diff_en, dac_12_13_diff_en, dac_10_11_diff_en, dac_8_9_diff_en, dac_6_7_diff_en, dac_4_5_diff_en, dac_2_3_diff_en, dac_0_1_diff_en
    ...
    def get_brdconfig(self):
        (_, _, _, _), (_, _, _, data_segment2_rx) = self.spi.read(self.REGISTER_MAP["BRDCONFIG"])
        ...
        return dac_15_brdcast_en, dac_14_brdcast_en, dac_13_brdcast_en, dac_12_brdcast_en, dac_11_brdcast_en, dac_10_brdcast_en, dac_9_brdcast_en, dac_8_brdcast_en, \
            dac_7_brdcast_en, dac_6_brdcast_en, dac_5_brdcast_en, dac_4_brdcast_en, dac_3_brdcast_en, dac_2_brdcast_en, dac_1_brdcast_en, dac_0_brdcast_en
    ...
    def get_syncconfig(self):
        (_, _, _, _), (_, _, _, data_segment2_rx) = self.spi.read(self.REGISTER_MAP["SYNCCONFIG"])
        ...
        return dac_15_sync_en, dac_14_sync_en, dac_13_sync_en, dac_12_sync_en, dac_11_sync_en, dac_10_sync_en, dac_9_sync_en, dac_8_sync_en, \
            dac_7_sync_en, dac_6_sync_en, dac_5_sync_en, dac_4_sync_en, dac_3_sync_en, dac_2_sync_en, dac_1_sync_en, dac_0_sync_en
    ...
    def get_toggconfig0(self):
        (_, _, _, _), (_, _, _, data_segment2_rx) = self.spi.read(self.REGISTER_MAP["TOGGCONFIG0"])
        ...
        return dac_15_ab_togg_en, dac_14_ab_togg_en, dac_13_ab_togg_en, dac_12_ab_togg_en, dac_11_ab_togg_en, dac_10_ab_togg_en, \
            dac_9_ab_togg_en, dac_8_ab_togg_en
    ...
    def get_toggconfig1(self):
        (_, _, _, _), (_, _, _, data_segment2_rx) = self.spi.read(self.REGISTER_MAP["TOGGCONFIG1"])
        ...
        return dac_7_ab_togg_en, dac_6_ab_togg_en, dac_5_ab_togg_en, dac_4_ab_togg_en, dac_3_ab_togg_en, dac_2_ab_togg_en, \
            dac_1_ab_togg_en, dac_0_ab_togg_en
    ...
    def get_dacpwdwn(self):
        (_, _, _, _), (_, _, _, data_segment2_rx) = self.spi.read(self.REGISTER_MAP["DACPWDWN"])
        ...
        return dac_15_pwdwn, dac_14_pwdwn, dac_13_pwdwn, dac_12_pwdwn, dac_11_pwdwn, dac_10_pwdwn, dac_9_pwdwn, dac_8_pwdwn, \
            dac_7_pwdwn, dac_6_pwdwn, dac_5_pwdwn, dac_4_pwdwn, dac_3_pwdwn, dac_2_pwdwn, dac_1_pwdwn, dac_0_pwdwn
    ...
    def get_brdcast(self):
        (_, _, _, _), (_, _, _, data_segment2_rx) = self.spi.read(self.REGISTER_MAP["BRDCAST"])
    ...
    def get_offset0(self):
        (_, _, _, _), (_, _, _, data_segment2_rx) = self.spi.read(self.REGISTER_MAP["OFFSET0"])
    ...
    def get_dacn(self, dac_index: int):
        if 0 <= dac_index <= 15:
            (_, _, _, _), (_, _, _, data_segment2_rx) = self.spi.read(self.REGISTER_MAP[f"DAC{dac_index}"])
            return data_segment2_rx
        else:
    ...
    def get_offset1(self):
        (_, _, _, _), (_, _, _, data_segment2_rx) = self.spi.read(self.REGISTER_MAP["OFFSET1"])
    ...
    def get_offset2(self):
        (_, _, _, _), (_, _, _, data_segment2_rx) = self.spi.read(self.REGISTER_MAP["OFFSET2"])
    ...
    def get_offset3(self):
        (_, _, _, _), (_, _, _, data_segment2_rx) = self.spi.read(self.REGISTER_MAP["OFFSET3"])
